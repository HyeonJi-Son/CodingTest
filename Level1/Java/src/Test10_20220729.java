...
public class Test10_20220729 {
    ...
    class Solution {
        public int[] solution(int[] arr) {
            ...
            /*
            -> solution:
            if(min > arr[i]) min = arr[i]; <-이 부분은 만약 min의 값이 현재 arr[i]에 저장된 값보다 작을 때만 동작한다.
            그러니까 arr[i]의 숫자가 min의 숫자보다 크면 지나가는 셈.
                - 최초로 반복문을 돌 때 integer최대 값보다 크거나 같은 수가 아닌 이상 무조건 1회 작동한다.
                - 그 다음 부터 min 값은 반복문을 돌고 if가 작동할 때마다 점점 작아진다.
                - 가장 마지막으로 저장된 min 값이 arr 인덱스에 저장된 것 중 가장 작은 값이다.
                - 그러므로 아래 remove에서 가장 작은 수가 제거되는 것이 가능해진다.
             */
